...
public class ParagraphVectorsServiceImpl implements ParagraphVectorsService {
	...
	public void trainNewFile(int docId, File inputTxt, String label) throws IOException {
		...
		ParagraphVectors vec = new ParagraphVectors.Builder()
				...
				.windowSize(5).iterate(iter).trainWordVectors(false).vocabCache(existVec.getVocab())
				.sequenceLearningAlgorithm(DM.class.getCanonicalName()).tokenizerFactory(token).vocabCache(cache)
				.sampling(0).build();
		try {
			vec.fit();
		} catch (Exception e) {
	...
	public void createModel(File inputTxt, String labelsource) {
		...
		ParagraphVectors vec = new ParagraphVectors.Builder().minWordFrequency(3).iterations(5).epochs(50)
				.layerSize(300).learningRate(0.025).labelsSource(source).windowSize(5).iterate(iter)
				.sequenceLearningAlgorithm(DM.class.getCanonicalName()).trainWordVectors(false).vocabCache(cache)
				.tokenizerFactory(token).sampling(0).build();
	...
	public double similarityTwoRawText(String text1, String text2) {
		...
		double lengthInf = (tokens1.size() < tokens2.size()) ? ((double) tokens1.size() / tokens2.size())
				: ((double) tokens2.size() / tokens1.size());
