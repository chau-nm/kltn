...
export const insert = async (
  thesisRegisterCalendar: ThesisRegisterCalendarModel,
): Promise<ThesisRegisterCalendarModel> => {
  ...
  const thesisRegisterCalendarResponse: ResponseModel<ThesisRegisterCalendarModel> =
    await http.post(
      ApiUrlConstants.INSERT_THESIS_REGISTER_CALENDAR,
      requestModel,
    );
...
export const disable = async (): Promise<boolean> => {
  const thesisRegisterCalendarResponse: ResponseModel<boolean> = await http.put(
    ApiUrlConstants.DISABLE_THESIS_REGISTER_CALENDAR,
  );
  return thesisRegisterCalendarResponse.data;
};
