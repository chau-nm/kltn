...
export const search = async (
  mutationParams: MutationParamsModel<OutlineReviewSearchConditionModel>,
...
export const insert = async (
  outlineComment: OutlineCommentModel,
): Promise<OutlineCommentModel> => {
  const requestModel: RequestModel<OutlineCommentModel> = {
    data: outlineComment,
  };
  const responseModel: ResponseModel<OutlineCommentModel> = await http.post(
    ApiUrlConstants.INSERT_OUTLINE_COMMENT,
    requestModel,
  );
...
export const updateComment = async (
  ...
  const responseModel: ResponseModel<boolean> = await http.put(
    ApiUrlConstants.UPDATE_OUTLINE_COMMENT,
    requestModel,
  );
...
export const remove = async (id: string): Promise<boolean> => {
  const responseModel: ResponseModel<boolean> = await http.delete(
    `${ApiUrlConstants.DELETE_OUTLINE_COMMENT_BY_THESIS}${id}`,
  );
